...
initial_r_diff = cfg.KT.initial_r_diff
...
def calibrate_r_diff_closed_form(correct_difficulty, incorrect_difficulty, correct_posterior1, incorrect_posterior1):
    ...
    for i in range(3):
        ...
        if sum_correct == 0 and sum_incorrect == 0:
            r_diff[i] = initial_r_diff[i]
            continue
    ...
    r_diff[0] = np.min([1-1e-6, r_diff[0]]) # easy
    r_diff[1] = np.min([r_diff[0], r_diff[1]]) # medium
    r_diff[2] = np.min([r_diff[1], r_diff[2]]) # hard
