...
if [ $USE_SGLANG -eq 1 ]; then
    python -m pip install "sglang[all]==0.5.2" --no-cache-dir && python -m pip install torch-memory-saver --no-cache-dir
fi
python -m pip install --no-cache-dir "vllm==0.11.0"

echo "2. install basic packages"
python -m pip install "transformers[hf_xet]>=4.51.0" accelerate datasets peft hf-transfer \
...
echo "python -m pip install git+https://github.com/ShaohonChen/PyExt.git@py311support"

python -m pip install "nvidia-ml-py>=12.560.30" "fastapi[standard]>=0.115.0" "optree>=0.13.0" "pydantic>=2.9" "grpcio>=1.62.1"
...
wget -nv https://github.com/Dao-AILab/flash-attention/releases/download/v2.8.1/flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl && \
    python -m pip install --no-cache-dir flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    python -m pip install "onnxscript==0.3.1"
